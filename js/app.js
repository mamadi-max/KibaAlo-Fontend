...
const Store = (() => {
  ...
  return {
    ...
    set:   (u) => {
      ...
      if (u.myShopId !== undefined && u.myShopId) localStorage.setItem('kba_shop_id', _s.myShopId);
      _L.forEach(f => f(_s));
    },
    ...
    try {
      ...
      if (!shopId) {
        try {
          ...
          if (shopId) { Store.set({ myShopId: shopId, dashStats: dash.data }); localStorage.setItem('kba_shop_id', shopId); }
        } catch { /**/ }
      }

      if (!shopId) {
        el.innerHTML = Views._emptyState('🏪','Boutique introuvable','Déconnectez-vous puis reconnectez-vous.');
        return;
      }
