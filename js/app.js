...
const DataLoader = {
  ...
  async myProducts() {
    ...
    try {
      ...
      if (!shopId) { el.innerHTML = Views._emptyState('🏪','Boutique introuvable','Déconnectez-vous puis reconnectez-vous.'); return; }
...
async function loadInitialData() {
  ...
  try {
    ...
    if (me.shopId) {
      Store.set({ myShopId: me.shopId });
      localStorage.setItem('kba_shop_id', me.shopId);
    }
    Router.updateTopbar();
