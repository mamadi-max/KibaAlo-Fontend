...
const Views = {
    ...
    el.innerHTML = `
      ...
      <div style="flex:1;overflow-y:auto;padding:20px 20px 40px;">
        ...
        <div class="field-group">
          ...
          <div class="field"><label>Téléphone</label><input id="f-phone" type="tel" placeholder="+226 70 00 00 00" autocomplete="tel"/></div>
          ${!isLogin ? `
            <div class="row-2">
  ...
  async _submitAuth(role, isLogin, el) {
    ...
    if (!phone) { showErr('⚠️ Numéro de téléphone requis'); return; }
    ...
    try {
      let res;
      if (isLogin) {
        res = await API.auth.login({ phone, password: pwd });
      } else {
  ...
  async _saveProduct() {
    ...
    if (!shopId) { Toast.show('⚠️ Aucune boutique associée'); return; }
    btn.classList.add('btn-loading'); btn.disabled = true;
    try {
